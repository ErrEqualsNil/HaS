...
def normalize_text(text: str, do_lowercase=False, add_title=False, do_normalize=False, title=None) -> str:
    assert (not add_title) or (title is not None)
    if add_title:
        text = f"{title}: {text}"
    if do_lowercase:
        text = text.lower()
...
def truncate_on_stop(responses: List[str], stop_list: List[str]) -> List[str]:
    truncated = []
    for resp in responses:
        for stop in stop_list:
            if stop in resp:
                resp = resp.split(stop)[0].strip()
...
def normalize_answer(s: str, lowercase: bool = True) -> str:
    ...
    if lowercase:
        s = _lower(s)
    s = _normalize(s)
    return _white_space_fix(_remove_articles(_remove_punc(s)))
